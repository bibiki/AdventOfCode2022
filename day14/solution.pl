...
%    start_a_grid(13, 11, Cave), sample_lines(Lines), init(Lines, Cave, Initialized), keep_sand_flowing(Initialized, 0, Result, [6, 0]).
    start_a_grid(535, 169, Cave), input(Lines), init(Lines, Cave, Initialized), keep_sand_flowing(Initialized, 0, Result, [500, 0]).
...
forward_free(Cave, [SandX, SandY]) :-
    Y is SandY + 1, cave_content(Cave, [SandX, Y], C), C = air.
left_free(Cave, [SandX, SandY]) :-
    SandX > 0,
    X is SandX - 1, Y is SandY + 1, cave_content(Cave, [X, Y], C), C = air.
right_free(Cave, [SandX, SandY]) :-
    SandX < 9,
    X is SandX + 1, Y is SandY + 1,  cave_content(Cave, [X, Y], C), C = air.
...
place_sand(Cave, [SandX, SandY], C) :-
    ...
    \+ left_free(Cave, [SandX, SandY]),
    \+ right_free(Cave, [SandX, SandY]),
    update_cave(Cave, [SandX, SandY], C).

place_sand(Cave, [SandX, SandY], C) :-
    forward_free(Cave, [SandX, SandY]), Y is SandY + 1, place_sand(Cave, [SandX, Y], C).
place_sand(Cave, [SandX, SandY], C) :-
    left_free(Cave, [SandX, SandY]), X is SandX + 1, Y is SandY + 1, place_sand(Cave, [X, Y], C).
place_sand(Cave, [SandX, SandY], C) :-
    right_free(Cave, [SandX, SandY]), X is SandX - 1, Y is SandY + 1, place_sand(Cave, [X, Y], C).
